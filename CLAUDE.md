...
| [docs/supabase_schema.sql](docs/supabase_schema.sql) | Idempotent SQL for initial schema — `profiles`, `renders`, `usage_events`, `waitlist`. Paste into Supabase SQL Editor once per project. |
| [docs/beta-roi-flow.md](docs/beta-roi-flow.md) | Marketing → activation → monetization sequence for the beta. The 5-step flow + open product decisions. Read before building any signup-followup email. |
